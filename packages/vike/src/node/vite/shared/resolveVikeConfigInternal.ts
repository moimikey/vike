...
async function getVikeConfigInternalOptional(): Promise<null | VikeConfigInternal> {
  if (!globalObject.vikeConfigPromise) return null
  const vikeConfig = await globalObject.vikeConfigPromise
  return vikeConfig
}
...
function hasViteConfigChanged(vikeConfigOld: VikeConfigInternal | null, vikeConfigNew: VikeConfigInternal): boolean {
  ...
  for (const configName of viteConfigNames) {
    ...
    // https://github.com/vikejs/vike/blob/1c6b7132e667a4359afc28d4601defb1579e8c58/packages/vike/src/node/vite/shared/resolveVikeConfigInternal/transpileAndExecuteFile.ts#L386
...
function assertMetaUsage(
  ...
  objectEntries(metaVal).forEach(([configName, def]) => {
    ...
    if (def.vite) {
      ...
      assertUsage(
        configEnv.config,
        `${errMsgBegin} ${pc.cyan(`meta.${configName}.env.config`)} to be ${pc.cyan('true')}`,
      )
    }
  })
}
